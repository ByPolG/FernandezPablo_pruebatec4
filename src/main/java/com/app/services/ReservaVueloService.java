...
public class ReservaVueloService {
    ...
    public ReservaVueloDTO guardarReserva(ReservaVueloDTO reservaVueloDTO) {
        ...
        // Verificamos el estado del vuelo
        if (vuelo.getEstado() == Vuelo.EstadoVuelo.NO_DISPONIBLE) {
            throw new RuntimeException("El vuelo no está disponible para reserva.");
        }
        ...
        // Actualizamos el estado del vuelo a NO_DISPONIBLE
        vuelo.setEstado(Vuelo.EstadoVuelo.NO_DISPONIBLE);  // Aquí usamos el enum
        vueloRepositorio.save(vuelo);

        // Devolvemos el DTO con la nueva reserva
        return new ReservaVueloDTO(
                reservaVuelo.getId(),
